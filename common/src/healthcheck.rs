use std::io;
use actix_web::{HttpResponse, get, web::Data, App, HttpServer, HttpResponseBuilder};
...
async fn check(health_check_fn: Data<HealthCheck>) -> HttpResponse {
    match health_check_fn.into_inner()() {
        Ok(success) => HttpResponseBuilder::new(StatusCode::OK).json(HealthCheckResponse{message:success}),
        Err(err) => HttpResponseBuilder::new(StatusCode::OK).json(HealthCheckResponse{message:err}),
    }
}
